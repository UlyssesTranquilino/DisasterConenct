...
export default function VolunteerMap({ 
    ...
    userLocation,
    selectedLocationId,
    helpRequests = []  // ✅ FIXED: Add this destructuring
}: VolunteerMapProps) {
    ...
    <div className="relative h-full w-full">
      ...
      >
        ...
        {helpRequests.map((request) => {
          ...
          if (!request.position || request.position[0] === 0 || request.position[1] === 0) {
            return null;
          }
          ...
          return (
            <Marker
              ...
              icon={createLucideMarker("#ef4444")} // Red marker
              eventHandlers={{
                click: () => onLocationSelect?.(request),
              }}
            >
              <Popup>
                <div className="text-neutral-800 min-w-[200px]">
                  <h3 className="font-bold text-base mb-1">{request.name}</h3>
                  <p className="text-xs text-neutral-500 mb-2">{request.location}</p>
                  {request.description && (
                    <div className="bg-neutral-100 p-2 rounded-lg mb-2">
                  ...
                  <button
                    ...
                    className="w-full mt-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium py-2 rounded-lg transition"
                  >
                    Accept this Request
                  </button>
                </div>
