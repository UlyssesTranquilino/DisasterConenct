...
    password: string,
    name: string,
    role: string, // Backend primary role (e.g., "civilian")
    profileData: any = {}
...
        profileData, // Contains the full role array { roles: ["civilian", "volunteer"] }
      }),
    });
  }
...
  async login(email: string, password: string, role: string): Promise<AuthResponse> {
    return this.request<AuthResponse>("/auth/login", {
      method: "POST",
      body: JSON.stringify({ email, password, role }), // <-- ROLE PASSED HERE
    });
  }
...
    idToken: string,
    role: string, // Backend primary role (e.g., "civilian")
    profileData?: any // Contains the full role array { roles: ["civilian", "volunteer"] }
  ): Promise<AuthResponse> {
...
      body: JSON.stringify({
        token: idToken,
        role, // Send as single role string (translated)
